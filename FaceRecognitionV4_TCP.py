...
already_sent = set()  # เก็บชื่อที่ส่งไปแล้วเพื่อไม่ส่งซ้ำ

# ===== รีเซตชื่อที่ส่งแล้วหากไม่มีใบหน้าในเฟรมต่อเนื่อง =====
not_found_counter = 0
NOT_FOUND_THRESHOLD = 20  # ประมาณ 1 วินาที (20 เฟรม)
...
while True:
    ...
    if process_this_frame:
        ...
        if face_locations:
            not_found_counter = 0  # รีเซตตัวนับเมื่อเจอหน้า
            try:
            ...
            for face_encoding in encodings:
                ...
                # ===== ส่งชื่อเฉพาะตอนเจอชื่อใหม่ และไม่ใช่ UNKNOWN =====
                if name != "UNKNOWN" and name not in already_sent:
                    try:
            ...
            if not_found_counter > NOT_FOUND_THRESHOLD:
                already_sent.clear()
                print("♻️ เคลียร์รายชื่อที่ส่งแล้ว เพราะไม่มีใครอยู่ในกล้อง")
