...
# ===== ตัวแปรรอ trigger จาก Server =====
send_triggered = False
send_lock = threading.Lock()

# ===== Thread รับคำสั่งจาก Server =====
def listen_trigger():
    global send_triggered
    while True:
        try:
            ...
            if data.lower() == "trig":
                with send_lock:
                    send_triggered = True
                    print("🟢 ได้รับ trigger จาก Server แล้ว")
        except Exception as e:
            print(f"❌ การรับ trigger ล้มเหลว: {e}")
            break

# ===== เริ่ม Thread รอฟัง trigger =====
threading.Thread(target=listen_trigger, daemon=True).start()
    ...
    if process_this_frame:
        ...
        if face_locations:
            ...
            for face_encoding in encodings:
                ...
                # ===== ส่งชื่อได้เฉพาะเมื่อได้รับ trigger =====
                with send_lock:
                    if send_triggered:
                        try:
                            client_socket.sendall(name.encode())
                            print(f"📤 ส่งชื่อหลังจาก trigger: {name}")
                            already_sent.add(name)
                        except Exception as e:
                            print(f"❌ ส่งชื่อไม่สำเร็จ: {e}")
                        send_triggered = False  # ส่งแค่ครั้งเดียวหลังจาก trigger

        else:
            not_found_counter += 1
            if not_found_counter > NOT_FOUND_THRESHOLD:
                already_sent.clear()
                print("♻️ เคลียร์รายชื่อเพราะไม่มีคนในกล้อง")
